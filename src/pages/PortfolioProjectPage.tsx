...
const PortfolioProjectPage: React.FC = () => {
    ...
    return (
        <div className="container mx-auto p-2 sm:p-4 max-w-6xl relative">
            {/* Coluna de Botões Funcionais */}
            <div className="hidden lg:flex absolute left-full top-16 ml-4 flex-col gap-2 z-20" style={{ pointerEvents: 'auto' }}>
            ...
            <div className="flex flex-col lg:flex-row gap-4">
                ...
                {/* Coluna de botões para mobile */}
                <div className="flex lg:hidden flex-row gap-4 justify-center mb-4">
                    {/* Botões de Curtir, Comentar, Conectar para mobile */}
                    <div className="flex flex-col items-center gap-1">
                        <button
                            onClick={handleLikeToggle}
                            className={`bg-black p-2 rounded-full shadow hover:bg-gray-800 flex items-center justify-center transform transition-all duration-200 font-bold text-base font-sans ${hasLiked ? 'scale-110' : 'scale-100'}`}
            ...
            <AddCollaboratorModal
                ...
                onInvite={async (user: Usuario) => {
                    try {
                        await apiClient.post('/conexoes', {
                            senderId: userId, // quem convida
                            recipientId: user.usuario_id, // convidado
                            projetoId: projeto?.projeto_id,
